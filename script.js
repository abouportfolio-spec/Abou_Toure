...
const typeWriterLoop = () => {
  const el = $('#typeWrap'); if(!el) return;
  const phrases = ['Développeur Web freelance —', 'Créateur de sites rapides —', 'Design & performance —', 'Interface claire & accessible —'];
  let pi=0, ci=0, forward=true, waiting=false, delay=70;

  const tick = () => {
    if(waiting) return;
    const str = phrases[pi];
    el.textContent = str.slice(0,ci);
    if(forward){ ci++; if(ci>=str.length){ waiting=true; setTimeout(()=>{forward=false; waiting=false;},900); } }
    else { ci--; if(ci<=0){ waiting=true; setTimeout(()=>{forward=true; pi=(pi+1)%phrases.length; waiting=false;},380); } }
  };
